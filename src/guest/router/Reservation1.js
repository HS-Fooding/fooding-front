...
const Reservation1 = () => {
    ...
    const calcTime = () => {
        const weekdaysWorkHour = JSON.parse(localStorage.getItem("weekdaysWorkHour"));
        const weekendsWorkHour = JSON.parse(localStorage.getItem("weekendsWorkHour"));
    ...
    const PeopleBtn = styled.button`
        background-color: ${(props) => (props.num == peopleNum ? "#FF7B54" : "white")};
        ...
        margin: 2.5px;
        border: none;
        border: ${(props) => (props.num == peopleNum ? "none" : "1px solid rgba(0,0,0,0.3)")};
        cursor: pointer;
    `;
                ...
                <PeopleContainer>
                    <PeopleInnerCon>
                        {peopleArr.map((num, index) => (
                            <PeopleBtn num={index + 1} key={index} onClick={() => peopleClick(num)}>
                                {num}명
                            </PeopleBtn>
                        ))}
                    </PeopleInnerCon>
